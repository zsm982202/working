...
Listener Listener::ListenTCP(uint16_t port) {
    ...
    Listener listener;
    listener.FromRawFd(fd); //设置fd_为监听套接字

    LOG_INFO("listen %d success...", port);
    XFiber::xfiber()->TakeOver(fd); //将fd的边沿模式的读写事件添加到efd_中
...
std::shared_ptr<Connection> Listener::Accept() {
    ...
    while (true) {
        ...
        if (client_fd > 0) {
            ...
            xfiber->TakeOver(client_fd); //将client_fd的边沿模式的读写事件添加到efd_中
            return std::shared_ptr<Connection>(new Connection(client_fd));
        }
        else {
            if (errno == EAGAIN) {
                // accept失败，协程切出
                xfiber->RegisterFdWithCurrFiber(fd_, -1, false); //永久注册读监听fd_（不会过期）
                xfiber->SwitchToSched(); //切到调度器
            }
            else if (errno == EINTR) {
    ...
    while (write_bytes < sz) {
        ...
        if (n > 0) {
            ...
            if (errno != EAGAIN && errno != EINTR) {
                ...
                xfiber->RegisterFdWithCurrFiber(fd_, expired_at, true); //注册客户端fd，为了接下来继续监听客户端读事件
                xfiber->SwitchToSched(); //切到调度器
            }
            else {
                //pass
